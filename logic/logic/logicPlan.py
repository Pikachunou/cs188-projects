...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    A1 = logic.PropSymbolExpr("PacmanAlive_1")
    A0 = logic.PropSymbolExpr("PacmanAlive_0")
    K0 = logic.PropSymbolExpr("PacmanKilled_0")
    B0  = logic.PropSymbolExpr("PacmanBorn_0")
    alive = A1 % ((A0 & ~K0 ) | (~A0 & B0))
    born = B0
    killed = ~(A0 & B0)
    return logic.conjoin(alive, killed, born)
    "*** END YOUR CODE HERE ***"
...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    if not hasattr(a, 'value'):
        a.value = True
    exec(f"result = {{{a.op}: a.value}}") #for some reason quotes kept on popping up and i was told to use exec to remove
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for i in combinations:
        Conjunctives.append(~i[0] | ~i[1])
    return logic.conjoin(Conjunctives)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    wall_restrictions = [PropSymbolExpr(wall_str, x, y) >> (~PropSymbolExpr(pacman_str, x, y, time=t)) for x,y in all_coords]
    actions = [PropSymbolExpr(action, time=t) for action in DIRECTIONS]
    pacphysics_sentences = []
    pacphysics_sentences.extend(wall_restrictions)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    KB[-1] = ~PropSymbolExpr(pacman_str, x1, y1, time=1)
...
def positionLogicPlan(problem) -> List:
    ...
    for i in range(50):
        print(i)
        pacman_coords = [PropSymbolExpr(pacman_str, x, y, time=i) for x, y in non_wall_coords]
        KB.append(exactlyOne(pacman_coords))
        goal_coords_expr = PropSymbolExpr(pacman_str, xg, yg, time=i)
        tmp = findModel(conjoin(KB) & goal_coords_expr)

        if tmp:
            action_seq = extractActionSequence(tmp, actions)
            return action_seq

        action_symbols = [PropSymbolExpr(action, time=i) for action in actions]
        KB.append(exactlyOne(action_symbols))
        successor_axioms = [pacmanSuccessorAxiomSingle(x, y, i+1, walls_grid) for x, y in non_wall_coords]
        KB.extend(successor_axioms)
...
def slam(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # Initialize KB and known_map at time 0
